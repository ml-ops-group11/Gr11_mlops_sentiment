...
train_raw = train_raw[train_raw["sentence"].str.len() > 0].drop_duplicates(subset="sentence")
...
train_df = train_raw.sample(n=min(TRAIN_SIZE, len(train_raw)), random_state=42).reset_index(drop=True)
val_df   = val_raw.sample(n=min(VAL_SIZE,   len(val_raw)),   random_state=42).reset_index(drop=True)
